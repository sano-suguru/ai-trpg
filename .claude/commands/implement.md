...
### 4.5. ユニットテスト・ミューテーションテスト

**対象:** `shared` または `api` に新規ビジネスロジックを追加した場合

```bash
# ユニットテスト実行
pnpm --filter @ai-trpg/shared test
pnpm --filter @ai-trpg/api test

# ミューテーションテスト（新規追加ファイル）
pnpm --filter @ai-trpg/shared mutation:file "src/path/to/new-file.ts"
pnpm --filter @ai-trpg/api mutation:file "src/path/to/new-file.ts"
```

**実施基準:**

| 変更内容 | ユニットテスト | ミューテーションテスト |
|----------|---------------|----------------------|
| `shared` に新規ロジック追加 | **必須** | **必須** |
| `api` に新規ユースケース追加 | **必須** | **必須** |
| `api` に新規リポジトリ追加 | 推奨 | 不要（DB依存） |
| `web` のUIコンポーネントのみ | 不要 | 不要 |
| 既存APIを使用するだけ | 不要 | 不要 |

**ミューテーションテストの目標:**

- ミューテーションスコア **80%以上** を目指す
- 生き残ったミュータントは意図的か確認し、必要なら追加テストを書く
...
3. **ユニットテスト結果**（Pass/Fail/Skip）- バックエンドロジック追加時のみ
4. **ミューテーションテスト結果**（スコア%/Skip）- バックエンドロジック追加時のみ
5. **E2Eテスト結果**（Pass/Fail）
6. **スクリーンショット**（Playwright MCPで撮影したもの）
7. **動画パス**（`apps/web/e2e-results/` 内）
